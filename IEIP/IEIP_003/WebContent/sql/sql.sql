...
select teacher_code, teacher_name, class_name, to_char(class_price, 'L999,999') class_price, substr(teach_resist_date,1,4)||'년'||substr(teach_resist_date,5,2)||'월'||substr(teach_resist_date,7,2)||'일' teach_resist_date from tbl_teacher_202201

select t.teacher_code, t.class_name, teacher_name, to_char(sum(c.tuition),'L999,999') tuition from tbl_teacher_202201 t, tbl_class_202201 c where t.teacher_code = c.teacher_code group by t.teacher_code, t.class_name, teacher_name order by t.teacher_code asc

select substr(c.resist_month,1,4)||'년'||substr(c.resist_month,5,2)||'월' resist_month, c.c_no, m.c_name, t.class_name, c.class_area, to_char(tuition, 'L999,999') tuition, m.grade from tbl_teacher_202201 t, tbl_member_202201 m, tbl_class_202201 c where t.teacher_code = c.teacher_code and m.c_no = c.c_no