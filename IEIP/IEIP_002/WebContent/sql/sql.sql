...
select h.hospaddr, case h.hospaddr when '10' then '서울' when '20' then '대전' when '30' then '대구' when '40' then '광주' end hospaddra,
...
select h.hospaddr, case h.hospaddr when '10' then '서울' when '20' then '대전' when '30' then '대구' when '40' then '광주' end hospaddra, count(v.hospcode) cnt from tbl_hosp_202108 h, tbl_vaccresv_202108 v where h.hospcode = v.hospcode group by h.hospaddr order by hospaddr asc

yy/mm/dd

select v.resvno, j.name, case substr(j.jumin,8,1) when '1' then '남' else '여' end gender, h.hospname, 
'20'||substr(v.resvdate,1,2)||'년'||substr(v.resvdate,4,2)||'월'||substr(v.resvdate,7,2)||'일' resvdate,
